...
import jp.co.sss.emanage.form.InfoSelectForm;
...
public class InfoSelectServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request,
        ...
        InfoSelectForm InfoSelectForm = new InfoSelectForm();

        // 条件検索キーを取得(ラジオボタンの値)
        InfoSelectForm.setFindKey(request.getParameter("radio"));
        ...
        // 条件検索キーが選択されていなかった場合
        if (InfoSelectForm.getFindKey() == null) {
            // 社員テーブルを検索し、全項目を取得する
        ...
        // 一般権限の社員ID検索表示を行う
        else if (Property.KEY_SELECT_BY_EMP_ID.equals(InfoSelectForm
                .getFindKey())) {
        ...
        // 一般権限の社員名検索表示を行う
        else if (Property.KEY_SELECT_BY_EMP_NAME.equals(InfoSelectForm
                .getFindKey())) {

            // フォームから社員名を取得し、社員名検索によるリスト取得
            InfoSelectForm.setEmpName(request.getParameter("empName"));
            empList = EmpDao.findByNameContains(InfoSelectForm.getEmpName());
        ...
        // 一般権限の部署ID検索表示を行う
        else if (Property.KEY_SELECT_BY_DEPT_ID.equals(InfoSelectForm
                .getFindKey())) {
            // フォームから部署IDを取得し、部署ID検索によるリスト取得
            InfoSelectForm.setDeptId(request.getParameter("deptId"));
            empList = EmpDao.findByDeptId(InfoSelectForm.getDeptId());
