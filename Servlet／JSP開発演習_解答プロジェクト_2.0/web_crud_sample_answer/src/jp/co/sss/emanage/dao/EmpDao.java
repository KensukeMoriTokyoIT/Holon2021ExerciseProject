...
public class EmpDao {
    ...
    /** employee(部署ID、部署名、所属人数)の検索SQL(所属人数がいなかった場合は表示しない) */
    // private static final String EMP_FIND_BY_DEPT =
    // "select employee.dept_id,dept_name, count(*) affiliation from employee inner join department on employee.dept_id = department.dept_id group by employee.dept_id,dept_name";
    ...
    /** employeeの変更SQL */
    private static final String EMP_UPDATE = "UPDATE employee SET emp_name = ?, gender = ?, address = ?, birthday = ?, authority = ?, dept_id = ? WHERE emp_id = ?";
    /** employeeの変更SQL 一般社員用 */
    private static final String EMP_SELF_UPDATE = "UPDATE employee SET emp_name = ?, gender = ?, address = ?, birthday = ?, dept_id = ? WHERE emp_id = ?";
    /** employeeのパスワード変更SQL */
    private static final String EMP_PASS_UPDATE = "UPDATE employee SET emp_pass = ? WHERE emp_id = ?";
    /** employeeの削除SQL */
    ...
    public static void insert(EmpBean empBean) {
        ...
        try {
            ...
            ps.setString(1, empBean.getEmpPass()); // パスワードを設定
            ps.setString(2, empBean.getEmpName()); // 社員名を設定
    ...
    public static void update(EmpBean empBean) {
        ...
        try {
            con = DBManager.getConnection();
            ps = con.prepareStatement(EMP_UPDATE);

            ps.setString(1, empBean.getEmpName()); // 社員名を設定
            ps.setString(2, empBean.getGender()); // 性別を設定
            ps.setString(3, empBean.getAddress()); // 住所を設定
            ps.setString(4, empBean.getBirthday()); // 生年月日を設定
            ps.setString(5, empBean.getAuthority()); // 権限を設定
            ps.setString(6, empBean.getDeptId()); // 部署IDを設定
            ps.setString(7, empBean.getEmpId()); // 変更する社員のIDを設定
     ...
     *            変更するの社員のbean
     */
    public static void selfUpdate(EmpBean empBean) {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = DBManager.getConnection();
            ps = con.prepareStatement(EMP_SELF_UPDATE);

            ps.setString(1, empBean.getEmpName()); // 社員名を設定
            ps.setString(2, empBean.getGender()); // 性別を設定
            ps.setString(3, empBean.getAddress()); // 住所を設定
            ps.setString(4, empBean.getBirthday()); // 生年月日を設定
            ps.setString(5, empBean.getDeptId()); // 部署IDを設定
            ps.setString(6, empBean.getEmpId()); // 変更する社員のIDを設定
    ...
    public static void delete(String empId) {
        ...
        try {
            con = DBManager.getConnection();
            ps = con.prepareStatement(EMP_DELETE);

            ps.setString(1, empId); // 削除する社員のIDを設定
     ...
     *            変更するの社員のbean
     */
    public static void empPassUpdate(EmpBean empBean) {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = DBManager.getConnection();
            ps = con.prepareStatement(EMP_PASS_UPDATE);

            ps.setString(1, empBean.getEmpPass()); // パスワードを設定
            ps.setString(2, empBean.getEmpId()); // 変更する社員のIDを設定
